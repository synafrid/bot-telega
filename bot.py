...
if not TELEGRAM_TOKEN:
    raise SystemExit(
        ...
        "Задайте переменную окружения или впишите токен для локального теста."
    )
...
def parse_report(pdf_bytes: bytes):
    """Принимает байты PDF. Возвращает (DataFrame, итог_из_pdf)."""
...
async def document_handler(message: types.Message) -> None:
    ...
    processing_msg = await message.answer(
        "📄 Обрабатываю ваш отчёт, это может занять несколько секунд..."
    )
    ...
    try:
        ...
        if df.empty:
            await processing_msg.edit_text(
                "❌ Не удалось найти данные в этом PDF. "
                "Проверьте, что это отчёт нужного формата."
            )
            return
        ...
        await message.answer_document(
            ...
            caption=(
                f"✅ Готово! Найдено {len(df)} позиций.\n"
                f"Сумма продаж: {df['Сумма продажи'].sum():,.2f} ₽"
            ),
        )
        ...
        await processing_msg.edit_text(
            f"⚠️ Произошла ошибка при обработке файла: {e}"
        )
